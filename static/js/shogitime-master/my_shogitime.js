...
将棋タイム.全局面構築 = function(指し手一覧, 初期局面){
    ...
    // for(var i = 0; i < 指し手一覧.length; i++){
        全局面[i] = [初期局面];
        if(i==0){
            var limit = Object.keys(指し手一覧[i]).length-1;
            // "勝敗"のkey分、要素数を1引いておく
        } else {
            var limit = Object.keys(指し手一覧[i]).length;
        }
...
将棋タイム.描画.指し手選択 = function ($){
    ...
    for(var i = 1; i < Object.keys(全指し手).length-1; i++){
...
将棋タイム.$次に移動ボタン_onclick = function(event){
    var 総手数 = Object.keys(this.全指し手[this.変化]).length-1 - 1;
